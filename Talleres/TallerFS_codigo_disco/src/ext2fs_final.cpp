...
{
	//TODO: Ejercicio 2
	// Obtengo el blockgroup en donde esta el inodo
	unsigned int blockgroup_number = blockgroup_for_inode(inode_number);
	// ..y su correspondiente descriptor
	Ext2FSBlockGroupDescriptor *desc_group = block_group(blockgroup_number);
	// Obtengo el comienzo de la tabla de inodos
	unsigned int table_start = desc_group->inode_table;
	
	// Obtengo el offset de la tabla de inodos del inodo correspondiente
	int inode_offset = blockgroup_inode_index(inode_number);
	
	// Valores necesarios
	unsigned int block_size = 1024<<superblock()->log_block_size;
	unsigned int inode_size = superblock()->inode_size;
	unsigned int cant_inodos_bloque = block_size / inode_size;
	
	// Obtengo el indice del blockgroup en donde esta el inodo y el indice del inodo dentro de ese blockgroup
	unsigned int indice_bloque_del_block_group = inode_offset / cant_inodos_bloque;
	unsigned int offset_del_bloque_del_inodo   = inode_offset % cant_inodos_bloque;

	// Obtengo el blockgroup donde esta el inodo
	unsigned char* table_buf = new unsigned char[block_size];
	read_block(table_start + indice_bloque_del_block_group, table_buf);
	
	// Devuelvo el inodo correspondiente de la tabla
	return &((Ext2FSInode *) table_buf)[offset_del_bloque_del_inodo];
}
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	//QUIERO obtener la LBA del bloque que me pasan por parametro

	unsigned int tam_bloque = 1024<<superblock()->log_block_size;
	
	unsigned char* block_buf = new unsigned char[tam_bloque];
	unsigned char* block_buf2 = new unsigned char[tam_bloque];
	
	//Cantidad de LBAs en un bloque
	unsigned int LBAs_por_bloque = tam_bloque / sizeof(unsigned int);
	
	if (block_number < 12) {
		return inode->block[block_number];
	}else if (block_number - 12 < LBAs_por_bloque){
		// SI el block_number es mayor que 12 y menor que las LBAS por bloque...
		
		// Me guardo un array de las LBAS del bloque de indireccion simple
		read_block(inode->block[12], block_buf);
		// Como se que no es parte de las 12 directas, debo restar asi puedo tener el offset de la tabla
		block_number -= 12;
		// Devuelvo la LBA correspondiente del offset nuevo 
		// Debo castear la matriz al formato de las LBAs (xq era arreglo de chars)
		return ((unsigned int *) block_buf)[block_number];
		
	}else if (block_number - 12 - LBAs_por_bloque < LBAs_por_bloque*LBAs_por_bloque){
		// Me guardo un array de las LBAS del bloque de indireccion doble
		read_block(inode->block[13], block_buf);
		// Resto todo lo directo y la indireccion simple
		block_number = block_number - 12 - LBAs_por_bloque;
		
		// QUIERO leer el bloque de indireccion simple correspondiente 
		// Obtengo el offset del bloque en el que se deberia encontrar
		unsigned int bloque_offset = int(block_number / LBAs_por_bloque);
		// Obtengo la LBA del bloque (casteada correctamente)
		unsigned int bloque = ((unsigned int *) block_buf)[bloque_offset];
		// Leo dicho bloque
		read_block(bloque, block_buf2);
		
		// Resto de block_number lo que ya no corresponde para obtener el offset dentro del bloque encontrado
		block_number -= bloque_offset * LBAs_por_bloque;
		// Casteo adecuadamente el array y devuelvo segun el offset nuevo guardado en block_number dentro de bloque
		return ((unsigned int *) block_buf2)[block_number];
		
	}else if (block_number - 12 - LBAs_por_bloque - LBAs_por_bloque*LBAs_por_bloque < 12 + LBAs_por_bloque*LBAs_por_bloque*LBAs_por_bloque){
		return 1;
		printf("Error3");
	}
