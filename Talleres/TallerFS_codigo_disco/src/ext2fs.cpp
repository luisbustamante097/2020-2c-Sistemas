...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	int inodo_num = blockgroup_inode_index(inode_number);
	
	unsigned int block_size = 1024<<superblock()->log_block_size;
	unsigned int inode_size = superblock()->inode_size;
	
	int cant_inodos_bloque = block_size / inode_size;
	int indice_bloque = int(inodo_num / cant_inodos_bloque);
	int indice_inodo = inodo_num % cant_inodos_bloque;

	//Leemos del table inode
	unsigned char* table_buf = new unsigned char[block_size];
	read_block(desc_group->inode_table + (indice_bloque),table_buf);
	
	//int cant_inodos_bloque = 1024<<superblock()->log_block_size / 1024<<superblock()->inode_size;
	//int indice = int(inode_number / cant_inodos_bloque);
	unsigned char * ret = new unsigned char[inode_size];
	ret = &table_buf[indice_inodo*inode_size];
	//free(table_buf);
	return (Ext2FSInode *) ret;
}
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	int cant_indirecto = tam_bloque / sizeof(unsigned int);
	
	if (block_number < 12) {
		return inode->block[block_number];
	}else if (block_number < 12 + cant_indirecto){
		// unsigned int * bloque_indirecto;
		// bloque_indirecto = (unsigned int*) inode->block[12];
		read_block(inode->block[12], block_buf);
		block_number -= 12;
		return block_buf[block_number * tam_bloque];
		
	}else if (block_number < 12 + cant_indirecto + cant_indirecto^2){
		//unsigned int bloque_indirecto;
		// bloque_indirecto = (unsigned int*) inode->block[13];
		read_block(inode->block[13], block_buf);
		block_number = block_number - 12 - cant_indirecto;
		read_block(block_buf[int(block_number / cant_indirecto) * tam_bloque], block_buf2);
		block_number -= (int(block_number / cant_indirecto)) * cant_indirecto;
		return block_buf2[int(block_number / cant_indirecto) * tam_bloque];
		
	}else if (block_number < 12 + cant_indirecto + cant_indirecto^2 + cant_indirecto^3){
		// unsigned int * bloque_indirecto;
		// bloque_indirecto = (unsigned int*) inode->block[14];
		// block_number = block_number - 12 - cant_indirecto - cant_indirecto^2;
		// bloque_indirecto = (unsigned int*) bloque_indirecto[int(block_number / cant_indirecto)];
		// block_number -= (int(block_number / cant_indirecto)) * cant_indirecto;
		// bloque_indirecto = (unsigned int*) bloque_indirecto[int(block_number / cant_indirecto)];
		// block_number -= (int(block_number / cant_indirecto)) * cant_indirecto;
		// return bloque_indirecto[block_number];
		return 1;
		printf("Error3");
	}
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	//TODO: Ejercicio 3
	//from es inodo de directorio
	//miramos sus bloques, Ext2FSDirEntry
	//en alguno vamos a encontrar DirEntry->name == filename, y devolvemos DirEntry->inode

	for (int i=0; i < 12; i++) {
		unsigned int * bloque = (unsigned int *) from->block[i];
		for (int j = 0; j < 512 / sizeof(unsigned int); j++) {
			Ext2FSDirEntry * dir_entry = (Ext2FSDirEntry *) bloque[j];
			if (dir_entry->name == filename) {
				return (Ext2FSInode *) dir_entry->inode;
			}
		}
	}

	unsigned int * bloque = (unsigned int *) from->block[12];
	for (int j = 0; j < 512 / sizeof(unsigned int); j++) {
		bloque = (unsigned int *) bloque[j];
		for (int i = 0; i < 512 / sizeof(unsigned int); i++) {
			Ext2FSDirEntry * dir_entry = (Ext2FSDirEntry *) bloque[i];
			if (dir_entry->name == filename) {
				return (Ext2FSInode *) dir_entry->inode;
			}
		}
	...
	bloque = (unsigned int *) from->block[13];
	for (int j = 0; j < 512 / sizeof(unsigned int); j++) {
		bloque = (unsigned int *) bloque[j];
		for (int i = 0; i < 512 / sizeof(unsigned int); i++) {
			bloque = (unsigned int *) bloque[i];
			for (int k = 0; k < 512; k++) {
				Ext2FSDirEntry * dir_entry = (Ext2FSDirEntry *) bloque[k];
				if (dir_entry->name == filename) {
					return (Ext2FSInode *) dir_entry->inode;
				}
			}
	...
	bloque = (unsigned int *) from->block[14];
	for (int j = 0; j < 512 / sizeof(unsigned int); j++) {
		bloque = (unsigned int *) bloque[j];
		for (int i = 0; i < 512 / sizeof(unsigned int); i++) {
			bloque = (unsigned int *) bloque[i];
			for (int k = 0; k < 512 / sizeof(unsigned int); k++) {
				bloque = (unsigned int *) bloque[k];
				for (int l = 0; l < 512 / sizeof(unsigned int); l++) {
					Ext2FSDirEntry * dir_entry = (Ext2FSDirEntry *) bloque[l];
					if (dir_entry->name == filename) {
						return (Ext2FSInode *) dir_entry->inode;
					}
				}
